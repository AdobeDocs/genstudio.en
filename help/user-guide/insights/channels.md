...
| **[!UICONTROL CPM]**         | Cost ($) for every one thousand ad impressions. <br>**Calculation**: (total amount `spent` divided by `impressions`) x 1000 = $ | A low value may indicate cost-effective visibility, especially when paired with a high click-through rate. |
